            ...
            if (e.type == pygame.JOYBUTTONDOWN):
                ...
                if e.button in [ControllerInput.BUTTON_A]:
                    ...
                    if self.current_menu_playlist == None:
                        ...
                        self.logger.info("Choosing plugin #%d in playlist #%d" % (
                        self.current_menu_playlist_entry_index, self.current_menu_playlist_index))
                        # Get out of the menu
                        self.leave_menu()
                    # Consume the event and return
                    return None
    ...
    def draw_frame(self, canvas):
        if self.in_menu:
            if self.current_menu_playlist == None:
                ...
                # are currently at
                self.logger.info("Drawing splash screen for playlist %d, plugin %d" % (
                self.current_menu_playlist_index, self.current_menu_playlist_entry_index))
