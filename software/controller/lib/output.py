...
class GuiOutput(Output):
    ...
    def draw_controller(self, drawable, centre, controller_number):
        ...
        if (controller_number in self.pressed_buttons and 0 in self.pressed_buttons[controller_number] and
                    self.pressed_buttons[controller_number][0] == True):
            x_colour = x_colour_pressed
        y_colour = y_colour_unpressed
        if (controller_number in self.pressed_buttons and 3 in self.pressed_buttons[controller_number] and
                    self.pressed_buttons[controller_number][3] == True):
            y_colour = y_colour_pressed
        a_colour = a_colour_unpressed
        if (controller_number in self.pressed_buttons and 1 in self.pressed_buttons[controller_number] and
                    self.pressed_buttons[controller_number][1] == True):
            a_colour = a_colour_pressed
        b_colour = b_colour_unpressed
        if (controller_number in self.pressed_buttons and 2 in self.pressed_buttons[controller_number] and
                    self.pressed_buttons[controller_number][2] == True):
        ...
        pygame.draw.circle(drawable, controller_colour, (int(centre_x - circle_centre_x), centre_y), int(circle_radius),
                           0)
        pygame.draw.circle(drawable, controller_colour, (int(centre_x + circle_centre_x), centre_y), int(circle_radius),
                           0)
        ...
        pygame.draw.circle(drawable, lighter_controller_colour, (int(centre_x - circle_centre_x), centre_y),
                           int(d_pad_diameter / 2.0 * 1.4), 0)

        # Draw the coloured buttons (becoming brighter when pressed)
        pygame.draw.circle(drawable, x_colour, (int(centre_x + circle_centre_x), int(centre_y - (x_to_b / 2))),
                           controller_button_radius, 0)
        pygame.draw.circle(drawable, b_colour, (int(centre_x + circle_centre_x), int(centre_y + (x_to_b / 2))),
                           controller_button_radius, 0)
        pygame.draw.circle(drawable, y_colour, (int(centre_x + circle_centre_x - (y_to_a / 2)), int(centre_y)),
                           controller_button_radius, 0)
        pygame.draw.circle(drawable, a_colour, (int(centre_x + circle_centre_x + (y_to_a / 2)), int(centre_y)),
                           controller_button_radius, 0)
    ...
    def update_floor_visualisation(self, drawable, canvas):
        ...
        for canvas_x in range(canvas_width):
            for canvas_y in range(canvas_height):
                ...
                pygame.draw.rect(drawable, canvas.get_pixel_tuple(canvas_x, canvas_y),
                                 pygame.Rect(canvas_x * pixels_per_cell + x_padding // 2,
                                             canvas_y * pixels_per_cell + y_padding // 2, pixels_per_cell,
                                             pixels_per_cell), 0)
