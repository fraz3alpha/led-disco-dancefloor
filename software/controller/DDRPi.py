...
class DDRPiMaster():
    ...
    def parse_commandline_arguments(self):
        ...
        parser.add_argument('--config', required=False, dest='config', default=None,
                            help='The location of the configuration file')
        # --testmode makes the script use a predefined set of config, so that --config is not required
        parser.add_argument('--testmode', required=False, dest='testmode', default=False,
                            help='Use a pre-configured set of options, useful for testing - only a GUI output, and a 24x18 cell floor',
                            action="store_true")
        # --headless overrides any enabled guis
        parser.add_argument('--headless', required=False, dest='headless', default=None, help='Don\'t bring up a GUI',
                            action="store_true")
        # --plugin defines a single plugin to be used, helpful in development and testing
        parser.add_argument('--plugin', required=False, dest='plugin', default=None,
                            help='The classname of the one plugin to use')
        parser.add_argument('--playlist', required=False, action='append', dest='playlist', default=None,
                            help='A playlist to use')
